...
# Comisión CoinEx spot: 0.2% entrada + 0.2% salida = 0.4% por trade
COMISION_COINEX = 0.004
...
def calcular_rsi(series, periodo=9):
    delta    = series.diff()
...
def calcular_atr(df, periodo=14):
    """Average True Range para TP/SL dinámico"""
    high  = df["high"]
    low   = df["low"]
    close = df["close"]
    tr1 = high - low
...
def get_klines_timeframe(market, period, limit=80):
    """Obtener velas de CoinEx"""
...
.cs-fee-banner {
    ...
    font-size: 11px; color: #ffa726; display: flex; align-items: center; gap: 8px;
}
...
.cs-signal-trend { background:rgba(77,166,255,.08); border:1px solid rgba(77,166,255,.3); border-radius:12px; padding:10px 14px; text-align:center; font-size:13px; font-weight:600; color:#4da6ff; margin:6px 0; }
.cs-signal-notif { background:rgba(255,167,38,.08); border:1px solid rgba(255,167,38,.25); border-radius:10px; padding:10px 14px; font-size:12px; color:#ffa726; margin:6px 0; }
...
.mt5-btn-buy { background:linear-gradient(135deg,#00b894,#00e676); color:#000; padding:14px; border-radius:6px; width:100%; text-align:center; font-family:'Rajdhani',sans-serif; font-size:18px; font-weight:700; cursor:pointer; border:none; }
...
.mt5-term-row { display:grid; grid-template-columns:85px 75px 55px 65px 65px 65px 70px; padding:7px 12px; border-bottom:1px solid #141414; font-size:10px; color:#888; align-items:center; }
.mt5-term-row:hover { background:#141414; }
.mt5-term-header { display:grid; grid-template-columns:85px 75px 55px 65px 65px 65px 70px; padding:5px 12px; background:#0f0f0f; font-size:9px; color:#444; letter-spacing:1px; text-transform:uppercase; border-bottom:1px solid #1e1e1e; position:sticky; top:0; }
...
if not st.session_state.auth:
    ...
    st.markdown(
        ...
        '<div style="background:#101010;border:1px solid #1e1e1e;border-radius:20px;padding:40px 32px;'
...
st.markdown(
    ...
    f'</div>'
    f'<div style="font-family:JetBrains Mono,monospace;font-size:11px;font-weight:700;color:{bot_badge_color};">'
    f'{bot_status}</div>'
    f'</div>',
    unsafe_allow_html=True
)
...
with col_m1:
    if st.button("🏠 HOME",      use_container_width=True, key="btn_home"): st.session_state.pagina = "HOME"
with col_m2:
    if st.button("⚡ LIVE",      use_container_width=True, key="btn_live"): st.session_state.pagina = "LIVE"
with col_m3:
    if st.button("📋 HISTORIAL", use_container_width=True, key="btn_hist"): st.session_state.pagina = "HISTORIAL"
...
if pagina == "HOME":
    st.markdown(
        ...
        '<div class="cs-desc">Automatiza tus operaciones en CoinEx con estrategia EMA + RSI + ATR + Tendencia Mayor. Sin emociones, 24/7.</div>'
        '<div class="cs-btns">'
    ...
    st.markdown(
        ...
        '<div class="cs-icon-item"><div class="cs-icon-box">🛡️</div><div class="cs-icon-lbl">Anti<br>Comision</div></div>'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    trades_demo = [
        ("12:55","BTC/USDT","COMPRA","+1.62%",True),
        ("13:10","ETH/USDT","COMPRA","+1.15%",True),
        ("13:28","SOL/USDT","COMPRA","+0.98%",True),
        ("13:45","XRP/USDT","COMPRA","+1.42%",True),
        ("14:02","BNB/USDT","COMPRA","+1.05%",True),
        ("14:20","DOGE/USDT","COMPRA","-1.30%",False),
    ]
    rows = ""
    for t in trades_demo:
        tag_cls = "cs-tl"
        pnl_cls = "cs-pos" if t[4] else "cs-neg"
        rows += (f'<div class="cs-trade"><span class="cs-arr">></span>'
                 f'<span class="cs-time">{t[0]}</span>'
                 f'<span class="cs-pair">{t[1]}</span>'
                 f'<span class="cs-tag {tag_cls}">{t[2]}</span>'
                 f'<span class="cs-pnl {pnl_cls}">{t[3]}</span></div>')
    st.markdown(
    ...
    st.markdown(
        ...
        '<div class="cs-feat-tag">TECNOLOGIA V2 — 4 FILTROS</div>'
        '<div class="cs-feat-h2">Estrategia mejorada para 30 USDT</div>'
        '<div class="cs-feat-p">5 filtros + ATR dinamico + comisiones CoinEx ya calculadas.</div>'
        '</div>'
        '<div class="cs-fcard"><div class="cs-ficon">📊</div><h3>4 Filtros de Entrada</h3><p>EMA7/18 + RSI(9) + Volumen + Tendencia Mayor (15min). Balance entre calidad y frecuencia de señales.</p></div>'
        '<div class="cs-fcard"><div class="cs-ficon">📐</div><h3>TP/SL Dinamico con ATR</h3><p>El Take Profit y Stop Loss se ajustan automaticamente segun la volatilidad real del mercado. Mas precision que valores fijos.</p></div>'
        '<div class="cs-fcard"><div class="cs-ficon">💸</div><h3>Comisiones CoinEx incluidas</h3><p>0.4% por trade (0.2% compra + 0.2% venta) ya descontado del calculo de TP/SL. El TP minimo es 1.2% para garantizar ganancia neta.</p></div>'
        '<div class="cs-fcard"><div class="cs-ficon">🕐</div><h3>Filtro de Horario</h3><p>El bot evita las horas de baja liquidez (2am-7am UTC) donde las senales falsas se multiplican.</p></div>'
        '<div class="cs-fcard"><div class="cs-ficon">💰</div><h3>Reinversion Automatica</h3><p>Capital crece con cada ganancia. Desde 30 USDT hacia 200 USDT reinvirtiendo todo en cada operacion.</p></div>'
        '<div class="cs-fcard"><div class="cs-ficon">📡</div><h3>Alertas Telegram</h3><p>Notificacion con precio de entrada, TP, SL y ganancia neta despues de comisiones.</p></div>'
        '</div>',
        unsafe_allow_html=True
    )
    st.markdown(
        '<div style="text-align:center;padding:26px 20px;border-top:1px solid #1e1e1e;color:#666;font-size:12px;">'
        '<div>2026 CRYPTOSCALPER BOT PRO v2. Todos los derechos reservados.</div>'
        '</div>',
        unsafe_allow_html=True
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with ctrl3:
        tf_mayor = st.selectbox("TF tendencia", ["5min","15min","1hour"], key="sel_tf_mayor")

    # Banner comisiones
    st.markdown(
        '<div class="cs-fee-banner">⚠️ <strong>Comisión CoinEx:</strong> 0.2% compra + 0.2% venta = 0.4% por trade. '
        'TP mínimo ajustado automáticamente para garantizar ganancia neta.</div>',
        unsafe_allow_html=True
    )
    ...
    with bc1:
        if st.button("▶ INICIAR",         use_container_width=True, key="btn_iniciar"):
            ...
            enviar_telegram(f"🤖 CRYPTOSCALPER v2 iniciado\nPar: {crypto} | TF: {timeframe} | Tendencia: {tf_mayor}")
    with bc2:
    ...
    if not st.session_state.bot_activo:
        ...
        try:
            # ── Datos timeframe principal ──────────────────────────────────
            candles_main  = get_klines_timeframe(market, timeframe, 80)
            df            = build_df(candles_main)

            # ── Indicadores principales ────────────────────────────────────
            df["EMA7"]    = df["close"].ewm(span=7).mean()
            df["EMA18"]   = df["close"].ewm(span=18).mean()
            df["RSI"]     = calcular_rsi(df["close"], 9)       # RSI 9 (mejorado)
            df["VOL_MA"]  = df["volume"].rolling(window=14).mean()
            df["ATR"]     = calcular_atr(df, 14)

            precio_actual = df["close"].iloc[-1]
            precio_prev   = df["close"].iloc[-2]
            vela_cierre   = df["close"].iloc[-1]
            vela_apertura = df["open"].iloc[-1]
            ema7          = df["EMA7"].iloc[-1]
            ...
            # ── Tendencia mayor (timeframe superior) ───────────────────────
            try:
                candles_mayor  = get_klines_timeframe(market, tf_mayor, 30)
                df_mayor       = build_df(candles_mayor)
                df_mayor["EMA21"] = df_mayor["close"].ewm(span=21).mean()
                df_mayor["EMA50"] = df_mayor["close"].ewm(span=50).mean()
                tendencia_alcista = (
                    df_mayor["EMA21"].iloc[-1] > df_mayor["EMA50"].iloc[-1] and
                    df_mayor["close"].iloc[-1] > df_mayor["EMA21"].iloc[-1]
                )
                tendencia_str = f"📈 ALCISTA ({tf_mayor})" if tendencia_alcista else f"📉 BAJISTA ({tf_mayor})"
                tendencia_color = "#00e676" if tendencia_alcista else "#e82929"
            except Exception:
                tendencia_alcista = False
                tendencia_str     = "⚠️ Sin datos tendencia"
                tendencia_color   = "#ffa726"

            # ── Filtro horario (evitar 2am-7am UTC) ───────────────────────
            hora_utc       = datetime.utcnow().hour
            hora_ok        = not (2 <= hora_utc < 7)
            hora_str       = f"✓ Hora OK ({hora_utc:02d}:xx UTC)" if hora_ok else f"✗ Baja liquidez ({hora_utc:02d}:xx UTC — espera 7am)"

            # ── Confirmación de vela (cerrar en verde) ─────────────────────
            vela_verde     = vela_cierre > vela_apertura

            # ── TP/SL dinámico basado en ATR ──────────────────────────────
            # ATR como % del precio actual
            atr_pct = (atr / precio_actual) * 100

            # TP = máximo entre 1.5x ATR y mínimo 1.8% (para cubrir comisión 0.4% + ganancia)
            tp_dinamico = max(atr_pct * 1.5, 1.8)
            sl_dinamico = max(atr_pct * 0.8, 0.9)

            # Ganancia y pérdida NETAS después de comisiones
            ganancia_neta_pct = tp_dinamico - (COMISION_COINEX * 100)
            perdida_neta_pct  = sl_dinamico + (COMISION_COINEX * 100)

            # Precios de TP y SL
            precio_tp = precio_actual * (1 + tp_dinamico / 100)
            precio_sl = precio_actual * (1 - sl_dinamico / 100)

            capital_op = st.session_state.capital

            # ── 5 FILTROS ─────────────────────────────────────────────────
            filtro_ema       = ema7 > ema18
            filtro_rsi       = 55 < rsi < 65
            filtro_volumen   = vol_actual > vol_promedio
            filtro_tendencia = tendencia_alcista

            todos_filtros    = (filtro_ema and filtro_rsi and filtro_volumen and
                                filtro_tendencia)
            filtros_ok       = sum([filtro_ema, filtro_rsi, filtro_volumen,
                                    filtro_tendencia])
            ...
            st.markdown(
                ...
                f'<div class="mt5-price-main {precio_color}">{precio_actual:,.4f} '
                f'<span style="font-size:14px;">{signo_cambio} {abs(cambio_pct):.2f}%</span></div></div>'
                f'<div class="mt5-quote-item"><div class="mt5-quote-lbl">ATR</div>'
                f'<div class="mt5-quote-val">{atr_pct:.2f}%</div></div>'
                f'<div class="mt5-quote-item"><div class="mt5-quote-lbl">TP DIN.</div>'
                f'<div class="mt5-quote-val" style="color:#00e676;">{tp_dinamico:.2f}%</div></div>'
                f'<div class="mt5-quote-item"><div class="mt5-quote-lbl">SL DIN.</div>'
                f'<div class="mt5-quote-val" style="color:#e82929;">{sl_dinamico:.2f}%</div></div>'
                f'<div class="mt5-quote-item"><div class="mt5-quote-lbl">NETO TP</div>'
                f'<div class="mt5-quote-val" style="color:#00e676;">+{ganancia_neta_pct:.2f}%</div></div>'
                f'<div class="mt5-quote-item"><div class="mt5-quote-lbl">CAPITAL</div>'
                f'<div class="mt5-quote-val" style="color:#4da6ff;">{capital_op:.2f} USDT</div></div>'
                f'</div>',
                unsafe_allow_html=True
            )
            ...
            st.markdown(
                ...
                f'<span style="color:{tendencia_color};font-weight:700;">{tendencia_str}</span> &nbsp;|&nbsp; '
                f'<span style="color:{"#00e676" if hora_ok else "#e82929"};">{hora_str}</span>'
                f'</div>',
                unsafe_allow_html=True
            )
            ...
            with col_right:
                ...
                # Indicadores
                rsi_pct   = min(rsi, 100)
                rsi_color = "#00e676" if filtro_rsi else ("#e82929" if rsi > 65 else "#ffa726")
                ...
                st.markdown(
                    ...
                    f'<div class="mt5-rp-row"><span class="mt5-rp-key">RSI(9)</span>'
                    ...
                    f'<span style="font-family:JetBrains Mono,monospace;font-size:11px;color:#ffa726;">{atr_pct:.2f}%</span></div>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                ...
                def frow(ok, label):
                    ...
                    return f'<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;"><span style="color:{c};font-size:11px;font-weight:700;">{ico}</span><span style="font-size:9px;color:#888;">{label}</span></div>'

                color_score = "#00e676" if filtros_ok >= 4 else ("#ffa726" if filtros_ok >= 2 else "#e82929")
                st.markdown(
                    f'<div class="mt5-rp-section"><div class="mt5-rp-title">FILTROS ({filtros_ok}/4) '
                    f'<span style="color:{color_score};">{"COMPRA" if todos_filtros else "ESPERA"}</span></div>'
                    + frow(filtro_ema,       "EMA 7 > EMA 18")
                    + frow(filtro_rsi,       f"RSI 55-65 ({rsi:.0f})")
                    + frow(filtro_volumen,   "Vol > Media")
                    + frow(filtro_tendencia, f"Tend. {tf_mayor} alcista")
                    + '</div>',
                    unsafe_allow_html=True
                )

                # Botón BUY con TP/SL dinámico
                st.markdown(
                    ...
                    f'<div class="mt5-btn-buy">BUY'
                    f'<span class="mt5-btn-sub">TP {precio_tp:,.4f} ({tp_dinamico:.2f}%)</span>'
                    f'<span class="mt5-btn-sub">SL {precio_sl:,.4f} ({sl_dinamico:.2f}%)</span>'
                    f'<span class="mt5-btn-sub" style="color:rgba(0,0,0,0.6);">Neto: +{ganancia_neta_pct:.2f}%</span>'
                    f'</div>'
                    f'</div>',
                    unsafe_allow_html=True
                )
            ...
            with col_chart:
                ...
                if st.session_state.en_posicion:
                    ...
                    precio_tp_pos  = entrada * (1 + tp_dinamico / 100)
                    precio_sl_pos  = entrada * (1 - sl_dinamico / 100)
                    ...
                    pnl_neto_pct   = ganancia_pct - (COMISION_COINEX * 100)
                    pnl_neto_usdt  = capital_op * pnl_neto_pct / 100

                    if precio_actual >= precio_tp_pos:
                        ganancia_real_pct   = tp_dinamico - (COMISION_COINEX * 100)
                        ganancia_real_usdt  = round(capital_op * ganancia_real_pct / 100, 4)
                        st.markdown(
                            f'<div class="cs-signal-tp">✅ TAKE PROFIT — VENDE AHORA<br>'
                            f'<span style="font-size:14px;">Ganancia neta: +{ganancia_real_pct:.2f}% (+{ganancia_real_usdt:.4f} USDT)</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                        if st.session_state.ultima_senal != "TP":
                            st.session_state.ultima_senal = "TP"
                            nuevo_capital = round(capital_op * (1 + ganancia_real_pct / 100), 4)
                            ...
                            st.session_state.historial.insert(0, {
                                ...
                                "pnl":           round(ganancia_real_pct, 2),
                                "pnl_bruto":     round(tp_dinamico, 2),
                                "comision":      round(COMISION_COINEX * 100, 2),
                                "resultado":     "TP",
                            ...
                            enviar_telegram(
                                f"✅ TAKE PROFIT\nPar: {crypto}\n"
                                f"Ganancia bruta: +{tp_dinamico:.2f}%\n"
                                f"Comisión: -0.40%\n"
                                f"Ganancia NETA: +{ganancia_real_pct:.2f}%\n"
                                f"Capital nuevo: {nuevo_capital:.2f} USDT\nVENDE AHORA"
                            )

                    elif precio_actual <= precio_sl_pos:
                        perdida_real_pct  = -(sl_dinamico + COMISION_COINEX * 100)
                        perdida_real_usdt = round(capital_op * abs(perdida_real_pct) / 100, 4)
                        st.markdown(
                            f'<div class="cs-signal-sl">🛑 STOP LOSS — VENDE AHORA<br>'
                            f'<span style="font-size:14px;">Pérdida neta: {perdida_real_pct:.2f}% (-{perdida_real_usdt:.4f} USDT)</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                        if st.session_state.ultima_senal != "SL":
                            st.session_state.ultima_senal = "SL"
                            nuevo_capital = round(capital_op * (1 + perdida_real_pct / 100), 4)
                            st.session_state.capital = nuevo_capital
                            st.session_state.historial.insert(0, {
                                "fecha":         datetime.now().strftime("%d/%m/%y %H:%M"),
                                "par":           crypto,
                                ...
                                "entrada":       round(entrada, 4),
                                "salida":        round(precio_actual, 4),
                                "pnl":           round(perdida_real_pct, 2),
                                "pnl_bruto":     round(-sl_dinamico, 2),
                                "comision":      round(COMISION_COINEX * 100, 2),
                                "resultado":     "SL",
                                "capital_usado": round(capital_op, 2),
                                "capital_nuevo": nuevo_capital,
                            ...
                            enviar_telegram(
                                f"🛑 STOP LOSS\nPar: {crypto}\n"
                                f"Pérdida bruta: -{sl_dinamico:.2f}%\n"
                                f"Comisión: -0.40%\n"
                                f"Pérdida NETA: {perdida_real_pct:.2f}%\n"
                                f"Capital nuevo: {nuevo_capital:.2f} USDT\nVENDE AHORA"
                            )
                    else:
                        st.markdown(
                            f'<div class="cs-signal-wait">EN POSICION — P&L bruto: '
                            f'<span style="color:{color_pnl};">{ganancia_pct:+.2f}%</span> &nbsp;|&nbsp; '
                            f'Neto: <span style="color:{color_pnl};">{pnl_neto_pct:+.2f}% ({pnl_neto_usdt:+.4f} USDT)</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                        # Info TP/SL en posición
                        dist_tp = ((precio_tp_pos - precio_actual) / precio_actual) * 100
                        dist_sl = ((precio_actual - precio_sl_pos) / precio_actual) * 100
                        st.markdown(
                            f'<div class="cs-signal-notif">'
                            f'TP: {precio_tp_pos:,.4f} (faltan {dist_tp:.2f}%) &nbsp;|&nbsp; '
                            f'SL: {precio_sl_pos:,.4f} (margen {dist_sl:.2f}%)'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                else:
                    if todos_filtros:
                        ganancia_usdt_esperada = round(capital_op * ganancia_neta_pct / 100, 4)
                        st.markdown(
                            f'<div class="cs-signal-buy">⚡ SEÑAL: COMPRA AHORA<br>'
                            f'<span style="font-size:13px;">TP: {precio_tp:,.4f} | SL: {precio_sl:,.4f} | '
                            f'Ganancia neta esperada: +{ganancia_neta_pct:.2f}% (+{ganancia_usdt_esperada:.4f} USDT)</span>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
                        if st.session_state.ultima_senal != "COMPRA":
                            ...
                            enviar_telegram(
                                ...
                                f"TP: {precio_tp:.4f} (+{tp_dinamico:.2f}%)\n"
                                f"SL: {precio_sl:.4f} (-{sl_dinamico:.2f}%)\n"
                                f"Ganancia neta esperada: +{ganancia_neta_pct:.2f}%\n"
                                f"(incluye comisión CoinEx 0.4%)"
                            )
                    else:
                        filtros_faltantes = 4 - filtros_ok
                        st.markdown(
                            f'<div class="cs-signal-wait">⏳ ESPERANDO SEÑAL — {filtros_ok}/4 filtros OK '
                            f'(faltan {filtros_faltantes})</div>',
                            unsafe_allow_html=True
                        )
                ...
                if st.session_state.en_posicion and st.session_state.precio_entrada > 0:
                    entrada_pos = st.session_state.precio_entrada
                    fig.add_hline(y=entrada_pos * (1 + tp_dinamico/100),
                        line_color="#00e676", line_dash="dash", line_width=1.5,
                        annotation_text=f"TP {tp_dinamico:.1f}%", row=1, col=1)
                    fig.add_hline(y=entrada_pos * (1 - sl_dinamico/100),
                        line_color="#e82929", line_dash="dash", line_width=1.5,
                        annotation_text=f"SL {sl_dinamico:.1f}%", row=1, col=1)
                    fig.add_hline(y=entrada_pos,
                ...
                fig.add_trace(go.Scatter(x=df.index, y=df["RSI"],
                    ...
                    line=dict(color="#e82929", width=1), name="RSI(9)"
                ), row=2, col=1)
            ...
            if hist_reciente:
                rows_term += (
                    ...
                    '<span>ENTRADA</span><span>SALIDA</span><span>NETO%</span><span>CAPITAL</span>'
                    '</div>'
                )
            ...
            st.markdown(
                ...
                '<div class="mt5-term-tab active">📋 Historial (P&L neto)</div>'
            ...
            st.success(f"🟢 Bot activo v2: {crypto} | TP: {tp_dinamico:.2f}% | SL: {sl_dinamico:.2f}% | Neto TP: +{ganancia_neta_pct:.2f}% | Capital: {capital_op:.2f} USDT")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if historial:
        ...
        pnl_neto   = round(sum([t["pnl"] for t in historial]), 2)
        comisiones = round(total * COMISION_COINEX * 100, 2)
        cap_actual = st.session_state.capital
        color_wr   = "#00e676" if win_rate >= 50 else "#e82929"
        color_pnl  = "#00e676" if pnl_neto >= 0 else "#e82929"
        pnl_str    = ("+" if pnl_neto >= 0 else "") + str(pnl_neto) + "%"
        ...
        st.markdown(
            ...
            f'<div style="font-size:10px;color:#555;margin-top:3px;">Comisiones pagadas: -{comisiones:.2f}% total</div>'
            f'</div>'
            ...
            f'<div style="font-size:11px;color:{color_pnl};">P&L neto: {pnl_str}</div>'
            f'</div></div>',
            unsafe_allow_html=True
        )
        st.markdown(
        ...
        st.markdown('<div class="cs-hist-seccion"><div class="cs-hist-seccion-titulo">Posiciones cerradas (P&L neto después de comisiones)</div></div>', unsafe_allow_html=True)
        ...
        for t in historial:
            ...
            trades_html += (
                ...
                f'<div style="font-size:10px;color:#444;margin-top:2px;">'
                f'Cap: {cap_usado} → {cap_nuevo} USDT • Com: -{comision_t}% • {t["fecha"]}</div>'
